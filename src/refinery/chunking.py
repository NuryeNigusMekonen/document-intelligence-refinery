    ...
    def run(self, extracted: ExtractedDocument) -> list[LogicalDocumentUnit]:
        ...
        for page in extracted.pages:
            ...
            for figure in page.figures:
                ...
                content_hash = self._hash_with_provenance(caption, page_refs)
                for ref in page_refs:
                    ref.content_hash = content_hash
                ldu = LogicalDocumentUnit(
                    ldu_id=deterministic_id("ldu", {"doc": extracted.doc_id, "page": page.page_number, "figure": caption}),
                    chunk_type="figure",
                    content=caption,
                    token_count=token_count(caption),
                    parent_section_path=section_path.copy(),
                    page_refs=page_refs,
                    content_hash=content_hash,
                )
